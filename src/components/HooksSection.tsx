...
export function HooksSection({ answers: a, setAnswers: setA }: SectionProps) {
            ...
            <SelectableRow key={h.id} selected={on}>
              <div className="flex items-center gap-2 p-2.5">
                ...
                >
                  ...
                  <span className="truncate text-sm font-medium text-ink-100">{h.label.fr}</span>
                </button>
                <button
                  type="button"
              ...
              {open && (
                <div className="space-y-1.5 border-t border-ink-700 px-3 py-2.5 text-xs leading-relaxed">
                  <p className="text-ink-300">{h.detail.fr}</p>
                  <p className="text-amber-flag">{h.caveat.fr}</p>
                </div>
              )}
            </SelectableRow>
          );
